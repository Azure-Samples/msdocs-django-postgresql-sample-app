...
class RestaurantRoutesTestCase(TestCase):
    ...
    def test_restaurant_review_page_loads(self):
        restaurant = Restaurant.objects.create(name='Test Restaurant', street_address='123 Test Street', description='Test Description')
    ...
    def test_add_restaurant(self):
        response = self.client.post(reverse('add_restaurant'),
            {'restaurant_name': 'Test Restaurant', 'street_address': '123 Test Street', 'description': 'Test Description'})
    ...
    def test_add_review(self):
        restaurant = Restaurant.objects.create(name='Test Restaurant', street_address='123 Test Street', description='Test Description')
        response = self.client.post(reverse('add_review', args=(restaurant.id,)),
...
class RestaurantModels(TestCase):
    ...
    def test_create_review(self):
        restaurant = Restaurant.objects.create(name='Test Restaurant', street_address='123 Test Street', description='Test Description')
        review = restaurant.review_set.create(user_name='Test User', rating=5, review_text='Test Review', review_date=datetime.datetime(2001, 1, 1))
        self.assertEqual(review.user_name, 'Test User')
        self.assertEqual(review.rating, 5)
        self.assertEqual(review.review_text, 'Test Review')
